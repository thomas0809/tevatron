...
python -m torch.distributed.launch --nproc_per_node=8 --master_port $MASTER_PORT -m tevatron.driver.train \
  ...
  --train_dir preprocessed/USPTO_50K_year/train_matched_rn.jsonl \
  --cache_dir cache/ \
...
python -m torch.distributed.launch --nproc_per_node=1 --master_port $MASTER_PORT -m tevatron.driver.encode \
  ...
  --p_max_len 256 \
  --dataset_name json \
  --encode_in_path preprocessed/USPTO_50K_year/corpus.jsonl \
...
do
  ...
  python -m torch.distributed.launch --nproc_per_node=1 --master_port $MASTER_PORT -m tevatron.driver.encode \
    ...
    --q_max_len 128 \
    --dataset_name json \
    --encode_in_path preprocessed/USPTO_50K_year/${split}.jsonl \
