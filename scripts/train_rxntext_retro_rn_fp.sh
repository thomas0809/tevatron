...
MODEL_DIR=output/rxntext_retro_rn_fp_b512_ep400
MASTER_PORT=$(shuf -n 1 -i 10000-65535)

python -m torch.distributed.launch --nproc_per_node=8 --master_port $MASTER_PORT -m tevatron.driver.train_custom_encoder \
  --output_dir ${MODEL_DIR} \
  --train_dir preprocessed/USPTO_50K/train_matched_rn.jsonl \
  --cache_dir cache/ \
  --data_cache_dir cache/data/ \
  --custom_model_name fingerprint_ffn \
...
#ckpt=
#
#python -m torch.distributed.launch --nproc_per_node=1 --master_port $MASTER_PORT -m tevatron.driver.encode_custom_encoder \
